...
# ============================
VERIFY = bool(environ.get('VERIFY', False))  # Verification Off (False) / On (True)
...
OWNERID = int(os.environ.get('OWNERID', '7753899951'))  # Replace with the actual admin ID
...
if 'DYNO' in environ:
    ON_HEROKU = True
    APP_NAME = environ.get('APP_NAME', 'your-app-name')  # Added default value
else:
    ON_HEROKU = False
    APP_NAME = None
...
# Fixed FQDN assignment with proper None handling
if not ON_HEROKU:
    FQDN = str(getenv('FQDN', BIND_ADRESS))
else:
    FQDN = f"{APP_NAME}.herokuapp.com" if APP_NAME else BIND_ADRESS
